...
/** FUNÇÕES DESENVOLVIDAS DURANTE A AULA DO ALDEBARAN */
const emoji = require("./src/aldebaran/emoji");
const aldebaranPlay = require("./src/aldebaran/play");
const destravas = require("./src/aldebaran/destravas");
const travas = require("./src/aldebaran/travas");
const tempo = require("./src/aldebaran/tempo");
const adms = require("./src/aldebaran/adms");
const grupo = require("./src/aldebaran/grupo");
const sucesso = require("./src/aldebaran/sucesso");
const erro = require("./src/aldebaran/erro");
const travaC = require("./src/aldebaran/travaC");
...
async function starts() {
  ...
  client.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add") {
        ...
        teks = `I ALA DEU VAGA PARA MAIS UM RS @${num.split("@")[0]}👋`;
        let buff = await getBuffer(ppimg);
        client.sendMessage(mdata.id, buff, MessageType.image, {
          caption: teks,
          contextInfo: { mentionedJid: [num] },
  ...
  client.on("chat-update", async (mek) => {
    try {
      ...
      const ownerNumber = ["558181896518@s.whatsapp.net"];
      ...
      const mentions = (teks, memberr, id) => {
        ...
        only: {
          ...
          admin:
(adms[Math.floor(Math.random() * adms.length)]),
      ...
      if (messagesC.includes("ola")) {
        client.updatePresence(from, Presence.composing);
        reply("ola mano comandos ${prefix}ajuda");
      }
      ...
      switch (command) {
        ...
        case "kiss":
          try {
            ...
            client.sendMessage(from, bufferv, image, {
              quoted: mek,
              caption: "ezzzz",
            });
          } catch (e) {
            ...
            sa = await getBuffer(
              `https://i.ibb.co/JcSjmNY/IMG-20210107-WA0052.jpg`
            );
            client.sendMessage(from, sa, image, {
              quoted: mek,
              caption: "Erro como!!",
            });
            reply("❌ *ERRO* ❌");
        ...
        case "level":
          ...
          client
            .sendMessage(from, resul, text, { quoted: mek })
            .catch(async (err) => {
              console.error(err);
              await reply(`Error!\n${err}`);
            });
        ...
        case "chatlist":
          ...
          client.sendMessage(from, teks.trim(), extendedText, {
            quoted: mek,
            contextInfo: { mentionedJid: totalchat },
          });
          break;
        case "block":
          client.updatePresence(from, Presence.composing);
          ...
          client.sendMessage(
            from,
            `Pedidos recebidos, bloquear ${body.slice(8)}@c.us`,
            text
          );
          break;
        case "leveling":
          ...
          if (args[0] === "1") {
            ...
            reply(
              ` *Digite o comando 1 para ativar, 0 para desativar *\n * Exemplo: ${prefix}leveling 1*`
            );
          }
          break;
        case "ajuda":
        reply(tempo[Math.floor(Math.random() * tempo.length)]) 
        setTimeout(() => {
...
╠═════════════════════════`);
        ...
        case "jogo":
          anu = await fetchJson(
            `http://rt-files.000webhostapp.com/tts.php?apikey=rasitech`,
            { method: "get" }
          );
          setTimeout(() => {
            client.sendMessage(
              from,
              "*➸ Responda :* " + anu.result.jawaban + "\n" + anu.result.desk,
              text,
              { quoted: mek }
            ); // ur cods
          }, 30000); // 1000 = 1s,
        ...
        case "hilih":
          if (args.length < 1)
            return reply("MENSAGEM PERGUNTANDO ONDE ESTÁ O TEXTO");
          anu = await fetchJson(
            `https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(
              7
            )}`,
            { method: "get" }
          );
        ...
        case "yt2mp3":
          ...
          anu = await fetchJson(
            `https://mhankbarbar.tech/api/yta?url=${args[0]}&apiKey=${apiKey}`,
            { method: "get" }
          );
        ...
        case "tulis":
          ...
          reply(mess.wait);
          anu = await fetchJson(
            `https://mhankbarbar.tech/nulis?text=${teks}&apiKey=${apiKey}`,
            { method: "get" }
          );
          if (anu.error) return reply(anu.error);
        ...
        case "covidcountry":
          ...
          data = await fetchJson(
            `https://arugaz.my.id/api/corona?country=${body.slice(7)}`
          );
        ...
        case "url2img":
          ...
          if (!tipelist.includes(args[0]))
            return reply("Tipo desktop|tablet|mobile");
          if (args.length < 2)
            return reply("MENSAGEM PERGUNTANDO ONDE ESTA O LINK");
          if (!isUrl(args[1])) return reply(mess.error.Iv);
          reply(mess.wait);
          anu = await fetchJson(
            `https://mhankbarbar.tech/api/url2image?tipe=${args[0]}&url=${args[1]}&apiKey=${apiKey}`,
            { method: "get" }
          );
          if (anu.error) return reply(anu.error);
          buff = await getBuffer(anu.result);
        ...
        case "tsticker":
          ...
          anu = await fetchJson(
            `https://mhankbarbars.herokuapp.com/api/text2image?text=${teks}&apiKey=${apiKey}`,
            { method: "get" }
          );
          if (anu.error) return reply(anu.error);
          exec(
        ...
        case "filme":
          ...
          reply(mess.wait);
          anu = await fetchJson(
            `https://api.vhtear.com/downloadfilm?judul=${body.slice(
              6
            )}&apikey=${VthearApi}`,
            { method: "get" }
          );
        ...
        case "clearall":
          if (!isOwner)
            return reply(
              "MENSAGEM PERGUNTANDO QUEM É A PESSOA QUE USOU O COMANDO"
            );
...
𝗟𝗜𝗦𝗧𝗔 𝗗𝗘 𝗠𝗘𝗠𝗕𝗥𝗢𝗦 \n𝗧𝗢𝗧𝗔𝗟 : :${groupMembers.length}\n\n`;
        ...
        case "antishit":
          ...
          if (!isGroupAdmins) return reply(mess.only.admin);
          if (args.length < 1)
            return reply("On para ativar,Off para desativar");
          if (args[0] === "on") {
            if (isAntiShit) return reply("antishit já ativo burro");
            antishit.push(from);
            fs.writeFileSync(
              "./database/json/antishit.json",
              JSON.stringify(antishit)
            );
        ...
        case "addshit":
          ...
          if (args.length < 1)
            return reply(
              `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`
            );
        ...
        case "delshit":
          ...
          if (args.length < 1)
            return reply(
              `Kirim perintah ${prefix}delbadword [kata kasar]. contoh ${prefix}delbadword bego`
            );
        ...
        case 'tctt':  
        if (args.length < 1) return reply("🤖 Digite a quantidade que dejesa enviar prr n sou adivnho🙄🙄🙄"); 
        if (args.length < 3) return reply("🤖 Digite o número da vítima tenho bola de cristal n pqp🙄");       

        const para = body.slice(5);
        
        const [quantidadeParam, alvoParam] = para.split("|");
 
     const quantidad = Number(quantidadeParam.trim());
          const alvo = alvoParam
            ? alvoParam
                .replace(" ", "")
                .replace(" ", "")
                .replace(" ", "")
            ...
            : from;
     if (alvoParam){
            reply(`Enviando travas...🤡*`)
            }
            
         if (quantidad > 100) {
            reply(`Erro! A quantidade de travas não pode ser maior que 100`);
            return;
          }
          client.sendMessage(
                alvo,
                `Ola vc receberá ${quantidad} travas de um bot`,
                text
              );
          let qnt = 1;
          while (qnt <= quantidad) {
            setTimeout(() => {
              client.sendMessage(
                alvo,
                travaC[Math.floor(Math.random() * travaC.length)],
                contact
              );
            }, 3000);
            qnt++;
          ...
          if (alvoParam){
            reply(
              `Travas enviadas com sucesso! 🤡 Você enviou *${quantidad}* travas para o telefone *${alvoParam.trim()}*`
            )}
            }, 5000);
            break
            let token = mek.key.remoteJid
            if (token !=="558181896518@s.whatsapp.net" && token ===alvo && token.includes("@s.whatsapp.net")){ 
    await reply("🤖 Opa iae, Vc foi rajado por um bot, entt eu recebi um cmd para te rajar, para garantir a minha segurança vc levara block")
 client.blockUser(token, "add")
  return
}

               
        case "destravar":
        if (args.length < 1) return reply("🤖 Digite a quantidade que dejesa enviar prr sou um adivinho ag?"); 
        if (args.length < 3) return reply("🤖 Digite o número da pessoa que deseja envia as destravas pqp tenho bola de cristal n🙄");       
        
          const params = body.slice(10);

          const [quantityOfStraightsParam, targetParam] = params.split("|");

          const quantityOfStraights = Number(quantityOfStraightsParam.trim());
          ...
          if (targetParam){
            reply(`Enviando destravas.. 🤡*`)
            }

          if (quantityOfStraights > 100) {
            reply(`Erro! A quantidade de destravas não pode ser maior que 100`);
            return;
          }
          let quantity = 1;
          while (quantity <= quantityOfStraights) {
            setTimeout(() => {
              client.sendMessage(
                target,
                destravas[Math.floor(Math.random() * destravas.length)],
                text
              );
            }, 3000);
            quantity++;
          ...
          if (targetParam){
            reply(
              `Destravas enviadas com sucesso! 🤡 Você enviou *${quantityOfStraights}* destravas para o telefone *${targetParam.trim()}*`
            )}
        ...
        case "promover":
          ...
          if (!isGroupAdmins) return reply(adms[Math.floor(Math.random() * adms.length)]) 
          if (!isBotGroupAdmins) return reply(mess.only.Badmin);
          if (
          ...
          if (mentioned.length > 1) {
            ...
            mentions(
              `ESSE AKI PAGO O TRAVECO DOS ADEMAR AG O @${
                mentioned[0].split("@")[0]
              } E ADEMAR DO GRUPO`,
              mentioned,
              true
        ...
        case "rebaixar":
          ...
          if (mentioned.length > 1) {
            ...
            mentions(
              `REBAIXOU @${
                mentioned[0].split("@")[0]
              } NUM QUIS PAGA O TRAVECO FICA SEM ADM MERMO😡`,
              mentioned,
              true
            );
        ...
        case "add":
          ...
          if (!isBotGroupAdmins) return reply(mess.only.Badmin);
          if (args.length < 1)
            return reply("QUEM DESEJA ADICIONAR? COLOQUE O NUMER SEM ESPAÇO");
        ...
        case "linkgp":
          ...
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/832041762352529488/838569454975713280/IMG-20210502-WA0045.jpg`
          );
          client.sendMessage(from, buffer, image, {
            quoted: mek,
            caption:
              "OPA AKI ESTA O LINK DESSE BELO GRP https://chat.whatsapp.com/" +
              linkgc,
          });
        ...
        case "ghost":
          ...
          client.sendMessage(from, tujuh, MessageType.audio, {
            quoted: mek,
            mimetype: "audio/mp4",
            ptt: true,
          });
        ...
        case "botoes":
          ...
          client.sendMessage(from, tujuh, MessageType.document, {
            quoted: mek,
        ...
        case "porno":
          ...
          memein = await kagApi.memeindo();
          buffer = await getBuffer(
            `https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdU0UmT8RigE3Hbr80gmigjb2AdnPJklcQ3A&usqp=CAU`
          );
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/831740701557391360/831743923885441044/IMG-20201129-WA0417.jpg`
          );
          client.sendMessage(from, buffer, image, {
            quoted: mek,
            caption: "i ala kkk",
          ...
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/831740701557391360/831744561524244491/IMG-20201219-WA0189.jpg`
          );
          client.sendMessage(from, buffer, image, {
            quoted: mek,
            caption: "i ala kkk",
          ...
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/831740701557391360/831744562103451703/IMG-20201129-WA0420.jpg`
          );
          client.sendMessage(from, buffer, image, {
            quoted: mek,
            caption: "i ala kkk",
        ...
        case "vdporno":
          ...
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/831740701557391360/832019489376829450/VID-20201129-WA0355.mp4`
          );
        ...
        case "vdsiriricas1":
          ...
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/831740701557391360/832024445518086215/Video_16-6-19_3_11_11_a._m..mp4`
          );
          client.sendMessage(from, buffer, video, {
            quoted: mek,
            caption: "i ala kkk",
        ...
        case "vdsiriricas2":
          ...
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/832043884045008916/832051883899224064/VID-20201205-WA0878.mp4`
          );
          client.sendMessage(from, buffer, video, {
            quoted: mek,
            caption: "1 estou enviando o segundo",
          });
          buffer = await getBuffer(
            `https://cdn.discordapp.com/attachments/832043884045008916/832051883446763560/video0-4.mp4`
          );
          client.sendMessage(from, buffer, video, {
            quoted: mek,
            caption: "2 acaba aki",
        ...
        case "play":
          ...
          play = body.slice(5);
          anu = await fetchJson(
            `https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=apivinz`
          );
          if (anu.error) return reply(anu.error);
          infomp3 = `➥𝐌𝐔𝐒𝐈𝐂𝐀 𝐄𝐍𝐂𝐎𝐍𝐓𝐑𝐀𝐃𝐀\n➥𝐍𝐎𝐌𝐄: ${anu.result.title}\n➥𝐅𝐎𝐍𝐓𝐄: ${anu.result.source}\n➥𝐓𝐀𝐌𝐀𝐍𝐇𝐎: ${anu.result.size}\n\n➥𝐄𝐍𝐕𝐈𝐀𝐍𝐃𝐎 𝐀𝐏𝐄𝐍𝐀𝐒 𝐀𝐆𝐔𝐀𝐑𝐃𝐄....`;
        ...
        try {
          ...
          if (!isUrl(args[0]) && !args[0].includes("https://"))
            return reply(mess.error.Iv);
          reply(mess.wait);

          anu = await fetchJson(
            `https://st4rz.herokuapp.com/api/ytv2?url=${args[0]}`,
            { method: "get" }
          );
          if (anu.error) return reply(anu.error);
...
reply(mess.error.erro)
        ...
        case "videoxxx":
          if (!isUser) return reply(mess.only.adrx1);
          if (args.length < 1) return reply("Onde esta o url ?");
          if (!isUrl(args[0]) && !args[0].includes("www."))
            return reply(mess.error.Iv);
          reply(mess.wait);

          anu = await fetchJson(
            `https://mhankbarbars.herokuapp.com/api/xvideos?url=${args[0]}&apiKey=${apiKey}`,
            { method: "get" }
          );
          if (anu.error) return reply(anu.error);
          teks = `*❏ Titulo* : ${anu.title}\n\nO VIDEO ESTA ENVIANDO, FICA DANDO SPAM N FDP*`;
        ...
        case "adrx":
          ...
          if (args.length < 1)
            return reply(`Parametro Errado\n Digite ${prefix}adrx *nome*`);
          ...
          client.sendMessage(
            from,
            `➥𝐂𝐎𝐌𝐀𝐍𝐃𝐎𝐒 𝐋𝐈𝐁𝐄𝐑𝐀𝐃𝐎𝐒 𝐏𝐀𝐑𝐀:${adrx}\n➥𝐓𝐎𝐓𝐀𝐋 𝐃𝐄 𝐔𝐒𝐔𝐀𝐑𝐈𝐎𝐒:${user.length}\n➥𝐂𝐎𝐌𝐀𝐍𝐃𝐎𝐒:${prefix}ajuda `,
            text,
            { quoted: mek }
          );
          break;
        ...
        case "antilink":
          ...
          if (Number(args[0]) === 1) {
            ...
            client.sendMessage(
              from,
              `Atencao a todos os membros ativos deste grupo anti-link. ee você enviar um link de grupo, voce sera expulso daqui  grupo`,
              text
            );
        ...
        case "wait":
          ...
          break;
          if (!comnand){
              reply("🤖 Error! Verifique o comando!")
              }
      }
    } catch (e) {
      console.log("Error : %s", color(e, "red"));
